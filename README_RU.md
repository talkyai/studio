<p align="center"><img src="public/icon.svg" alt="TalkyAI Studio Logo" width="200" height="200"></p>

# TalkyAI Studio — локальный и облачный AI‑ассистент
[![publish](https://github.com/talkyai/studio/actions/workflows/release.yml/badge.svg)](https://github.com/talkyai/studio/actions/workflows/release.yml)

## Платформа следующего поколения для оркестрации ИИ

TalkyAI Studio — это единое настольное приложение для удобного управления рабочими процессами ИИ как через облачные API, так и через локальные LLM‑серверы. Мгновенно переключайтесь между провайдерами и сохраняйте конфигурации в виде переиспользуемых проектов.

> Кроссплатформенно • Конфиденциально • Удобно для разработчиков  
> Поддерживает Windows, macOS и Linux. Все данные и ключи хранятся локально.

![TalkyAI Studio Interface](public/screen1.png)
![TalkyAI Studio Interface](public/screen2.png)

## ✨ Ключевые возможности

### 🗨️ Чат: Markdown и уведомления
- Встроенный рендер Markdown в чате (заголовки, списки, инлайн/блочные коды, ссылки, выделения)
- Встроенные уведомления в приложении (стек Snackbar в правом верхнем углу) для действий и сообщений плагинов

### 🧱 Конструктор промптов (Builder)
- Сборка запроса из готовых блоков: Роль, Контекст (загрузка TXT), Инструкция, Few-shot Примеры, Переменные ввода, Ограничения, Формат вывода
- Холст с drag-and-drop: изменение порядка, дублирование, удаление, сворачивание блоков
- Живой предпросмотр автоматически форматирует итоговый промпт и показывает примерный счётчик токенов
- Переменные через плейсхолдеры {{name}} подставляются в предпросмотр и при выполнении
- Кнопка Выполнить отправляет собранный промпт в выбранную LLM и переключает на Чат
- Примечание: поддерживается загрузка TXT; парсинг PDF/DOCX появится в следующих версиях

### 🚀 Поддержка нескольких провайдеров
- Облачные API:
  - DeepSeek API (включена дефолтная конечная точка)
  - Сервисы, совместимые с OpenAI
- Локальные инференс‑серверы:
  - llama.cpp (оптимизации для CPU/GPU)
  - Ollama (автоматическое управление моделями)

### ⚡ Установка и управление локальными серверами в один клик
- Проверка совместимости бинарников с вашей системой (CPU/ARM/CUDA/HIP/Vulkan)
- Загрузка прямо из приложения с определением ОС
- Мониторинг статуса: Загрузка → Распаковка → Готово
- Автозагрузка модели для Ollama

### 🧠 Расширенное управление моделями
- Для Ollama:
  - Локальный браузер моделей
  - Отслеживание прогресса pull
  - Пользовательские параметры чата через JSON
- Для llama.cpp:
  - Настройка GGUF‑репозитория
  - Настройка порта сервера
  - Тонкая настройка генерации

### 💼 Рабочие пространства проектов
- Сохранение целых конфигураций (модель, провайдер, настройки сервера)
- Активация в один клик
- Истории чатов на уровне проектов

## 🖥️ Системная панель
Виджет мониторинга в реальном времени показывает:
- Использование CPU/RAM
- Статус сервера
- Активный проект

## 🌍 Интернационализация
- Языки: English | Русский
- Темы: светлая/тёмная
- Все предпочтения сохраняются локально

## 🚀 Быстрый старт

```mermaid
graph TD
    A[Запуск приложения] --> B{Выбор режима}
    B -->|Облако| C[Настроить API]
    B -->|Локально| D[Настроить сервер]
    C --> E[Перейти в чат]
    D --> E
```

1. Выберите режим:
   - Облако: укажите конечные точки API и ключи
   - Локально: установите необходимые компоненты сервера

2. Для локального режима:
```bash
# Пример для Ollama
ollama pull llama3
```

3. Начните общение:
   - Используйте инструменты разработчика
   - Прикрепляйте файлы/папки как контекст
   - Применяйте шаблоны подсказок

## 🔧 Дополнительная конфигурация

### Параметры генерации
| Параметр       | Описание                                  | По умолчанию |
|----------------|-------------------------------------------|--------------|
| `temperature`  | Контроль креативности                     | 0.7          |
| `top_k`        | Ширина выбора токенов                     | 40           |
| `max_tokens`   | Лимит длины ответа (−1 = без ограничений) | −1           |

### Пример JSON для Ollama API
```json
{
  "options": {
    "num_ctx": 4096,
    "seed": 42,
    "stop": ["</s>"]
  }
}
```

## 🔒 Конфиденциальность и безопасность
- Нулевая телеметрия
- Локальное шифрование чувствительных данных
- Безопасное хранение ключей через системные хранилища ОС

## 📦 Системные требования
- Минимальные:
  - 8 ГБ RAM
  - 2 ГБ свободного места
- Рекомендуемые для локальных моделей:
  - 16+ ГБ RAM
  - Видеокарта NVIDIA/AMD с 8+ ГБ VRAM

## 📚 Документация для разработчиков
- API плагинов (RU): docs/api_ru.md
- Plugin API (EN): docs/api.md

## 🛠 Разработка и сборка

### Предварительные требования
- Node.js 18+ и npm
- Rust (stable) через rustup
- Системные зависимости Tauri: https://tauri.app/v2/guides/getting-started/prerequisites
  - Windows: C++ Build Tools (Visual Studio), WebView2 Runtime
  - macOS: Xcode Command Line Tools
  - Linux: GTK3 и WebKit2GTK 4.1. Пример (Debian/Ubuntu):
    ```bash
    sudo apt update && sudo apt install -y libwebkit2gtk-4.1-0 libgtk-3-0 librsvg2-2 build-essential curl wget file libssl-dev
    ```

### Установка зависимостей
```bash
npm install
```

### Запуск дев-версии (в десктопном окне)
```bash
npx tauri dev
```
Это запустит:
- Vite dev‑сервер на http://localhost:1420
- Окно Tauri с поддержкой горячей перезагрузки

### Сборка установочных пакетов
```bash
npx tauri build
```
Готовые артефакты будут в src-tauri/target для вашей платформы (например, .exe/.msi на Windows, .app/.dmg на macOS, .deb/.AppImage/.rpm на Linux).

### Опционально: сборка Linux‑артефактов в Docker (с Windows/macOS)
```bash
npm run build:linux:docker
```
Требуется Docker Desktop; сборка выполнится по Dockerfile.linux.

## 📜 Лицензия
Проект распространяется по лицензии MIT — см. LICENSE.md для подробностей.

---

### Почему именно TalkyAI Studio?
✅ Единый интерфейс — управляйте всеми AI‑процессами в одном месте  
✅ Портативность — локальный режим не зависит от облака  
✅ Визуальный Конструктор промптов — собирайте запросы из блоков с предпросмотром  
✅ Расширяемость — система плагинов с UI‑площадками (см. docs/api_ru.md)

---
